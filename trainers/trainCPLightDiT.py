...
class CPLightDiTTrainer(Trainer):
    def __init__(
        self,
        config: DictConfig,
        model: nn.Module,
        ...
        self.stage = str(config.get("stage", "cplightdit_finetune"))
    ...
    def _apply_trainable_policy(self) -> None:
        ...
        if self.stage == "ray_pretrain":
            ...
            return
        if self.stage != "cplightdit_finetune":
            raise ValueError(f"Unsupported stage='{self.stage}'. Expected 'cplightdit_finetune' or 'ray_pretrain'.")
    ...
    def _print_trainable_parameter_counts(self) -> None:
        if self.rank != 0:
            return
        model = unwrap_model(self.models["model"])
        print(f"CP-LightDiT parameters: total={count_parameters(model):,}, trainable={count_trainable_parameters(model):,}")
    ...
    def save_checkpoint(self, epoch: int, name: str | None = None) -> None:
        ...
        if self.rank != 0 or self.stage != "ray_pretrain":
            return
        path = self.result_dir / (name or f"ray_encoder_epoch_{epoch:04d}.pth")
        torch.save({"light_encoder": unwrap_model(self.models["light_encoder"]).state_dict(), "epoch": epoch}, path)
