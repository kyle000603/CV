...
- `losses/`: flow MSE and light-transfer auxiliary losses.
...
```bash
python train.py -cn TrainCPLightDiT
```
...
```bash
CUDA_VISIBLE_DEVICES=2,3 torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:0 --nproc_per_node=2 train.py -cn TrainCPLightDiT
```
...
For local smoke tests without downloading VIDIT or SiT:

```bash
python train.py -cn TrainCPLightDiT debug_one_batch=true batch_size=2 assets.hf_vidit.enabled=false assets.vidit.enabled=false assets.sit_pretrained.enabled=false
```
...
- `PhysicsLightTransfer`: directional-light shading prior using depth-derived normals, ambient light, and log-shading transfer.
- `LightTransferTransformer`: patch transformer that refines dense light transfer and outputs a 10-channel dense condition.
