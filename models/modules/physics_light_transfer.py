...
class PhysicsLightTransfer(nn.Module):
    ...
    def forward(
        ...
        source_light = source_light.to(source_image.device)
        target_light = target_light.to(source_image.device)
        source_direction = self._direction_3d(source_light, ray=source_ray)
        target_direction = self._direction_3d(target_light, ray=target_ray)
        source_ray_map = source_direction.view(batch, 3, 1, 1).expand(-1, -1, height, width)
        target_ray_map = target_direction.view(batch, 3, 1, 1).expand(-1, -1, height, width)
        source_response = (normal * source_ray_map).sum(dim=1, keepdim=True).clamp_min(0.0)
        target_response = (normal * target_ray_map).sum(dim=1, keepdim=True).clamp_min(0.0)
        h_s = self._shading(normal, source_light, ray=source_direction)
        h_t = self._shading(normal, target_light, ray=target_direction)
        ...
        return {
            ...
            "source_ray_map": torch.nan_to_num(source_ray_map),
            "target_ray_map": torch.nan_to_num(target_ray_map),
            "source_response": torch.nan_to_num(source_response),
            "target_response": torch.nan_to_num(target_response),
        }
